...
const clearCurrentExpiredSessions = `-- name: ClearCurrentExpiredSessions :exec
DELETE FROM session
WHERE expires_at < now() AND is_blocked = false
`

func (q *Queries) ClearCurrentExpiredSessions(ctx context.Context) error {
	_, err := q.db.Exec(ctx, clearCurrentExpiredSessions)
	return err
}
