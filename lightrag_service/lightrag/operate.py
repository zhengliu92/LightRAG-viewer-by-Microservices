...
async def extract_entities(
    ...
    if example_number and example_number < len(PROMPTS["entity_extraction_examples"]):
        examples = PROMPTS["entity_extraction_examples"][int(example_number)]
    else:
    ...
    async_lock = asyncio.Lock()  # Create a lock

    async def _process_single_content(
        chunk_key_dp: tuple[str, TextChunkSchema], callback=None
    ):
        ...
        async with async_lock:
            if callback is not None:
                callback(
                    incremental_percent=50 / len(ordered_chunks),
                    status="正在提取实体和关系",
                )
    ...
    for result in tqdm_async(
        asyncio.as_completed(
            [_process_single_content(c, callback) for c in ordered_chunks]
        ),
...
async def kg_query(
    ...
    if example_number and example_number < len(PROMPTS["keywords_extraction_examples"]):
        examples = PROMPTS["keywords_extraction_examples"][int(example_number)]
    else:
    ...
    context, sources = await _build_query_context(
        keywords,
    ...
    if kb_id:
        redis_client.set(
            f"context:{kb_id}:{query}", json.dumps(sources), ex=60 * 60 * 24 * 7
        )
...
"""
    sources = {
        "entities_context": entities_context.split("\r\n")[:-1],
        "relations_context": relations_context.split("\r\n")[:-1],
        "text_units_context": text_units_context.split("\r\n")[:-1],
    }
