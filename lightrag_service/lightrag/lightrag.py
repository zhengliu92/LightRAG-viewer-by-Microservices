...
from app.models import KBFigure, KBTable
...
from prompt.prompt import GRAPH_FIELD_SEP
import random
...
class LightRAG:
    ...
    def __post_init__(self):
        ...
        self.assets_kv = self.key_string_value_json_storage_cls(
            namespace="assets",
            global_config=asdict(self),
            embedding_func=None,
        )
        ...
        self.figure_vdb = self.vector_db_storage_cls(
            namespace="figures",
            global_config=asdict(self),
            embedding_func=self.embedding_func,
            meta_fields={"content", "doc_id"},
        )

        self.table_vdb = self.vector_db_storage_cls(
            namespace="tables",
            global_config=asdict(self),
            embedding_func=self.embedding_func,
            meta_fields={"content", "table_html", "doc_id"},
        )
    ...
    def insert_figure(
        ...
        run_async(self.ainsert_figure, figure)
        # loop = always_get_an_event_loop()
        # return loop.run_until_complete(self.ainsert_figure(figure))

    def insert_table(
        self,
        table: KBTable,
    ):
        run_async(self.ainsert_table, table)
        # loop = always_get_an_event_loop()
        # return loop.run_until_complete(self.ainsert_table(table))

    async def ainsert_figure(self, figure: KBFigure):
        figure_data = {
            figure.img_id: {
                "content": figure.caption,
                "doc_id": figure.kbfile_id,
                "type": "figure",
            }
        }
        await self.assets_kv.upsert(figure_data)
        await self.assets_kv.index_done_callback()
        await self.figure_vdb.upsert(figure_data)
        await self.figure_vdb.index_done_callback()

    async def adelete_figure(self, doc_id: str):
        figures_to_delete = await self.assets_kv.filter(
            lambda x: x["doc_id"] == doc_id and x["type"] == "figure"
        )
        if not figures_to_delete:
            return
        await self.figure_vdb.delete(list(figures_to_delete.keys()))
        await self.figure_vdb.index_done_callback()
        await self.assets_kv.delete(list(figures_to_delete.keys()))
        await self.assets_kv.index_done_callback()

    async def adelete_table(self, doc_id: str):
        tables_to_delete = await self.assets_kv.filter(
            lambda x: x["doc_id"] == doc_id and x["type"] == "table"
        )
        if not tables_to_delete:
            return
        await self.table_vdb.delete(list(tables_to_delete.keys()))
        await self.table_vdb.index_done_callback()
        await self.assets_kv.delete(list(tables_to_delete.keys()))
        await self.assets_kv.index_done_callback()

    async def ainsert_table(
        self,
        table: KBTable,
    ):
        table_data = {
            table.table_id: {
                ...
                "doc_id": table.kbfile_id,
                "type": "table",
            }
        }
        await self.assets_kv.upsert(table_data)
        await self.assets_kv.index_done_callback()
        await self.table_vdb.upsert(table_data)
        await self.table_vdb.index_done_callback()

    def query_figure(self, query: str, top_k: int = 10, threshold: float = 0.5):
        loop = always_get_an_event_loop()
        return loop.run_until_complete(self.aquery_figure(query, top_k, threshold))

    async def aquery_figure(self, query: str, top_n: int = 10, threshold: float = 0.5):
        datas = await self.figure_vdb.query(query, top_k=top_n)
        return [
            {
                "id": d["id"],
                "score": round(d["distance"], 2),
                "content": d["content"],
            }
            for d in datas
    ...
    def query_table(self, query: str, top_n: int = 10, threshold: float = 0.5):
        loop = always_get_an_event_loop()
        return loop.run_until_complete(self.aquery_table(query, top_n, threshold))

    async def aquery_table(self, query: str, top_n: int = 10, threshold: float = 0.5):
        datas = await self.table_vdb.query(query, top_k=top_n)
        return [
            {
                "id": data["id"],
                "score": round(data["distance"], 2),
    ...
    def insert(self, string_or_strings, callback=None, kbfile_id: Optional[str] = None):
        if not callback:
            callback = lambda **kwargs: None
        run_async(self.ainsert, string_or_strings, callback, kbfile_id)

        # loop = always_get_an_event_loop()
        # return loop.run_until_complete(self.ainsert(string_or_strings, callback))
    ...
    async def ainsert(
        ...
        get_doc_id = lambda content: (
            compute_mdhash_id(content, prefix="doc-") if not kbfile_id else kbfile_id
        )
            ...
            for doc_id, doc in tqdm_async(
                ...
                try:
                    ...
                    try:
                        ...
                        callback(
                            incremental_percent=random.randint(5, 7),
                            status="正在构建知识图谱，请耐心等待",
                        )
                    ...
                    callback(
                        incremental_percent=random.randint(3, 7),
                        status="正在构建索引",
                    )
    ...
    def query(self, query: str, kb_id: str = None, param: QueryParam = QueryParam()):
        loop = always_get_an_event_loop()
        return loop.run_until_complete(self.aquery(query, kb_id, param))

    async def aquery(
        self, query: str, kb_id: str = None, param: QueryParam = QueryParam()
    ):
    ...
    async def adelete_by_doc_id(self, doc_id: str):
        ...
        try:
            ...
            # 8. Delete figures and tables
            await self.adelete_figure(doc_id)
            await self.adelete_table(doc_id)
