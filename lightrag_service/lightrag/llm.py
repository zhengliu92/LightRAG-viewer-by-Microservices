...
async def bedrock_complete_if_cache(
    ...
    **kwargs,
    ...
    try:
        ...
        raise ImportError("Please install aioboto3 before initialize bedrock backend.")
...
def initialize_hf_model(model_name):
    try:
        ...
        raise ImportError("Please install transformers before initialize hf backend.")
...
async def ollama_model_if_cache(
    ...
    **kwargs,
    ...
    try:
        ...
        raise ImportError("Please install ollama before initialize ollama backend.")
...
async def openai_embedding(
    ...
    openai_async_client = (
        AsyncOpenAI(api_key=api_key, project=project)
        if base_url is None
        else AsyncOpenAI(base_url=base_url, api_key=api_key, project=project)
    )
    try:
...
async def hf_embedding(texts: list[str], tokenizer, embed_model) -> np.ndarray:
    try:
        ...
        raise ImportError("Please install torch before initialize hf backend.")
